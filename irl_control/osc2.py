...
from transforms3d.quaternions import qconjugate, qinverse
...
class OSC():
    ...
    def calc_error(self, target: Target, device: Device):
        ...
        if np.sum(device.ctrlr_dof_abg) > 0:
            ...
            q_r = np.array(qmult(device.get_state(DeviceState.EE_QUAT), qinverse(target.get_quat())))
    ...
    def generate(self, targets: Dict[str, Target]):
        ...
        for device_name, target in targets.items():
            ...
            if np.all(target_vel == 0):
                ist, c1, c2 = np.intersect1d(device.joint_ids_all, self.robot.joint_ids_all[mask], return_indices=True)
        ...
        if self.use_g:
            u_all += self.sim.data.qfrc_bias[self.robot.joint_ids_all[mask]]
