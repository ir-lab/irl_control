...
class GaussianPolicy:
    def __init__(self, observation_space, action_space, hidden_sizes, activation, output_activation, sess):
        self.sess = sess
        with tf.variable_scope('pi'):
            self.obs_ph, self.a_ph = core.placeholders_from_spaces(observation_space, action_space)
            self.adv_ph, self.logp_old_ph = core.placeholders(None, None)
            
            self.act_dim = self.a_ph.shape.as_list()[-1]
            self.mu = core.mlp(self.obs_ph, list(hidden_sizes)+[self.act_dim], activation, output_activation)
            ...
            self.logp = core.gaussian_likelihood(self.a_ph, self.mu, self.log_std)
            self.logp_pi = core.gaussian_likelihood(self.pi, self.mu, self.log_std)
            old_mu_ph, old_log_std_ph = core.placeholders(self.act_dim, self.act_dim)
            self.d_kl = core.diagonal_gaussian_kl(self.mu, self.log_std, old_mu_ph, old_log_std_ph)
    ...
    def sample_actions(self, obs, deterministic=False):
        if deterministic:
            mu_action = self.sess.run(self.mu, feed_dict = {self.obs_ph : obs})
        else:
            mu_action = self.sess.run(self.pi, feed_dict = {self.obs_ph : obs})
        return mu_action
...
class ValueFunc:
    def __init__(self, observation_space, action_space, hidden_sizes, activation, output_activation, sess):
        ...
        with tf.variable_scope('v'):
            self.obs_ph, _ = core.placeholders_from_spaces(observation_space, action_space)
            self.v = tf.squeeze(core.mlp(self.obs_ph, list(hidden_sizes)+[1], activation, output_activation), axis=1)
...
def trpo(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0, 
    ...
    gp = GaussianPolicy(env.observation_space, env.action_space, hidden_sizes=(64,64), activation=tf.tanh, output_activation=None, sess=sess)

    vf = ValueFunc(env.observation_space, env.action_space, hidden_sizes=(64,64), activation=tf.tanh, output_activation=None, sess=sess)
    ...
    def sim_mp(policy_fn=None, obsfeat_fn=None):
        ...
        num_sa = 0
        while True:
            t = sim_single(policy_fn, obsfeat_fn, max_ep_len)
            trajs.append(t)
            num_sa += len(t)
    ...
    traj_batch = sim_mp(policy_fn=gp.sample_actions, obsfeat_fn= lambda obs : obs)
